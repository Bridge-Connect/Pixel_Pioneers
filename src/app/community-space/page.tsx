...
export default function CommunitySpacePage() {
  ...
  useEffect(() => {
    ...
    try {
      const postsRef = collection(db, "communitySpacePosts")
  ...
  const validateFile = (file: File, type: "image" | "video"): boolean => {
    ...
    if (file.size > maxSize) {
      ...
      showToast(`File too large. Max size: ${maxSizeMB}MB`, "error")
      return false
    }
  ...
  const uploadFile = async (file: File, type: "image" | "video"): Promise<string> => {
    ...
    const fileName = `community-space-media/${user.uid}/${timestamp}-${file.name}`
  ...
  const handleSubmitPost = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, "communitySpacePosts"), postData)
  ...
  const handleReply = async (postId: string, content: string) => {
    ...
    try {
      ...
      await addDoc(collection(db, `communitySpacePosts/${postId}/replies`), replyData)
  ...
  const handleReact = async (postId: string, type: "like" | "heart") => {
    ...
    try {
      const postRef = doc(db, "communitySpacePosts", postId)
  ...
  return (
    <AuthGuardComponent>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        <HeaderNavigation user={user} title="Community Space" subtitle="Share & Connect" />
        ...
        <main className="max-w-4xl mx-auto p-4 pb-32">
          ...
          <div className="text-center mb-8 mt-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              <h2 className="text-3xl font-bold text-gray-800 mb-4">🧑‍🤝‍🧑 Community Space</h2>
